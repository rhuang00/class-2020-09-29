...
```{r}
d_2019 <- 
  read_excel("raw_data/class_enrollment_summary_by_term_12.20.19.xlsx", skip = 3) %>% 
  clean_names() %>% 
 rename_with(~ str_replace(.x, "course_", "")) %>% 
  select(id, title, department, name, u_grad) %>% 
 filter(u_grad > 10)
...
d_2018 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.24.18.xlsx", 2)

 d_2017 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.20.17.xlsx", 2)

d_2016 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_fall_2016.xlsx", 0)
